...
class Enemy{
    ...
    isClicked(xMouse, yMouse){
        if (xMouse > this.x - this.w && xMouse < this.x + this.w &&
            yMouse > this.y - this.w && yMouse < this.y + this.w) {
            return true;
        }
        return false;
    }
...
export default class Game extends React.Component {
    ...
    componentDidMount() {
        ...
        this.canvas.addEventListener('click', (event) => {
            ...
            if (this.state.status === "menu") {
                if (x > WIDTH / 2 - BUTTON_WIDTH / 2 && x < WIDTH / 2 + BUTTON_WIDTH / 2 &&
                    y > HEIGHT / 2 - BUTTON_HEIGHT / 2 && y < HEIGHT / 2 + BUTTON_HEIGHT / 2) {
                    this.start();
                }
            }else {
    ...
    start = () => {
        ...
        let max = (this.state.score > this.state.maxScore) ? this.state.score : this.state.maxScore;
    ...
    render() {
        return (
            <Canvas ref={this.cRef} width={WIDTH} height={HEIGHT}></Canvas>
        )
    }
}